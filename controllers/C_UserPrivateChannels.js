...
exports.addPrivateChannel = function (req, res) {
    ...
    var tchannel = new M_USERPRIVATECHANNEL({
    ...
    if (req.body.fb_token !== undefined && req.body.fb_token != null && req.body.fb_token != '') {
        ...
        M_USER.find({ fb_token: req.body.fb_token }, function (err, users) {
            ...
            if (err) {
                ...
                tchannel.host_user_id = host_user._id;

                result = tchannel.ValidateModel(req.body.host_lat, req.body.host_lon);
                
                if (result.is_success) {

                    //Validamos que el amigo exista en el sistema
                    M_USER.findById(tchannel.user_id, function (err, friendUser) {
                        ...
                        if (err) {
                            ...
                            M_GROUP.findById(tchannel.group_id, function (err, group) {
                                if (err) {
                                    ...
                                    M_USERGROUP.find({ group_id: group._id, user_id: tchannel.host_user_id, is_validated: true }, function (err, userHostExistOnGroup) {
                                        if (err) {
                                            result.status_code = Messages.Generals.ServerError.status_code;
                                            result.error_code = Messages.Generals.ServerError.error_code;
                                            result.message = err.message;
                                            result.is_success = Messages.Generals.ServerError.is_success;
                                            res.status(200).jsonp(result);
                                        } else if (userHostExistOnGroup.length == 0) {
                                            ...
                                            M_USERGROUP.find({ group_id: group._id, user_id: tchannel.user_id, is_validated: true }, function (err, userFriendExistOnGroup) {
                                                if (err) {
                                                    result.status_code = Messages.Generals.ServerError.status_code;
                                                    result.error_code = Messages.Generals.ServerError.error_code;
                                                    result.message = err.message;
                                                    result.is_success = Messages.Generals.ServerError.is_success;
                                                    res.status(200).jsonp(result);
                                                } else if (userFriendExistOnGroup.length == 0) {
                                                    ...
                                                    M_CHANNEL.find({ _id: tchannel.channel_id, group_id: tchannel.group_id }, function (err, channels) {
                                                        ...
                                                        if (err) {
                                                            ...
                                                            if (Utilities.CalcDistance(req.body.host_lat, req.body.host_lon, parseFloat(friendUser.location_lat), parseFloat(friendUser.location_lon)) <= 0.300) {
                                                                ...
                                                                M_USERPRIVATECHANNEL.find({ user_id: friendUser._id, channel_id: tchannel.channel_id, group_id: tchannel.group_id }, function (err, duplicatedFriends) {
                                                                    if (err) {
                                                                        result.status_code = Messages.Generals.ServerError.status_code;
                                                                        result.error_code = Messages.Generals.ServerError.error_code;
                                                                        result.message = err.message;
                                                                        ...
                                                                        var channelValidation = channel.ValidateFilters(friendUser);
                                                                        if (channelValidation.is_success) {
                                                                            tchannel.save(function (err, privateChannel) {
                                                                                if (err) {
                                                                                    result.status_code = Messages.Generals.ServerError.status_code;
                                                                                    result.error_code = Messages.Generals.ServerError.error_code;
                                                                                    result.message = err.message;
                                                                                    ...
                                                                                    result.privateChannel = privateChannel;
                                                                                }
                                                                                res.status(200).jsonp(result);
                                                                            });

                                                                        } else {
                                                                            result.status_code = channelValidation.status_code;
                                                                            result.error_code = channelValidation.error_code;
                                                                            result.message = channelValidation.message;
                                                                            result.is_success = channelValidation.is_success;
                                                                            res.status(200).jsonp(result);
                                                                        }
                                                                    }
                                                                });
    ...
    if (req.body.fb_token !== undefined && req.body.fb_token != null && req.body.fb_token != '') {
        ...
        M_USER.find({ fb_token: req.body.fb_token }, function (err, users) {
            ...
            if (err) {
                ...
                tchannel.user_id = users[0]._id;
                tchannel.host_user_id = tchannel.user_id;
                var tmpResult = tchannel.ValidateModel('0', '0');

                //Validamos los datos requeridos
                if (result.is_success) {

                    //Validamos que el grupo exista
                    M_GROUP.findById(tchannel.group_id, function (err, grupo) {
                        ...
                        if (err) {
                            ...
                            M_USERGROUP.find({ user_id: tchannel.user_id, group_id : tchannel.group_id}, function (err, items) {
                                if (err) {
                                    result.status_code = Messages.Generals.ServerError.status_code;
                                    result.error_code = Messages.Generals.ServerError.error_code;
                                    result.message = err.message;
                                    ...
                                    M_CHANNEL.findById(tchannel.channel_id, function (err, channel) {
                                        if (err) {
                                            result.status_code = Messages.Generals.ServerError.status_code;
                                            result.error_code = Messages.Generals.ServerError.error_code;
                                            result.message = err.message;
                                            ...
                                            M_USERPRIVATECHANNEL.find({ user_id: tchannel.user_id, channel_id: tchannel.channel_id, group_id : tchannel.group_id}, function (err, items) {
                                                if (err) {
                                                    result.status_code = Messages.Generals.ServerError.status_code;
                                                    result.error_code = Messages.Generals.ServerError.error_code;
                                                    result.message = err.message;
                                                    ...
                                                    items[0].remove(function (err) {
                                                        if (err) {
                                                            ...
                                                            M_USERPRIVATECHANNEL.find({ channel_id: tchannel.channel_id, group_id: tchannel.group_id }, function (err, items) {
                                                                if (err) {
                                                                    result.status_code = Messages.Generals.ServerError.status_code;
                                                                    result.error_code = Messages.Generals.ServerError.error_code;
                                                                    result.message = err.message;
                    ...
                    res.status(200).jsonp(tmpResult);
                }
            }
        });
